...
public class ProductController {
    ...
    public ResponseEntity<ProductResponseDTO> getProductById(
        ...
        // Track recently viewed only when user is logged in
        if (userId != null) {
            try {
                recentlyViewedService.addViewedProduct(userId, id);
            } catch (Exception e) {
                log.warn("Failed to track recently viewed for user {}: {}", userId, e.getMessage());
            }
        }
    ...
    public ResponseEntity<ProductSearchResponseDTO> searchProducts(
        ...
        return ResponseEntity.ok(
                service.searchProducts(keyword, category, brand, minPrice, maxPrice, tag, page, size, sortBy, sortDir)
        );
    }
}
